...
Хэдеры имеют следующую структуру  

unsigned int - размер секции в байтах вместе с хэдером  
Строка (символы до нулевого байта) - имя хэдера  

Типы хэдеров:  

MAIN - основной хэдер, из него скорей всего состоит весь файл  
INFO - дополнительная информация, относящая к файлу  
MATERIALLIST - список используемых материалов  
MATERIAL - материал  
TRACK - информация о трэке и какая-то коллекция моделей  
MODEL - модель, которая может состоять из батчей (и может быть чего-то еще)  
BATCH - набор полигонов с одной тектурой 
BATCH2 - набор полигонов с одной тектурой  
HIERARCHY - коллекция объектов на трэке (моделей, камер, освещения, сцены)  
SCENE - какая-то информация о сцене  
MESH - меш, состоящий из модели и доп информации о ней  
CAMERA2 - камера  
LIGHT - свет  


Данные по каждому и видов хэдера:
...
unsigned int - не знаю что, может быть какой-то размер  
string - наверно название формата (BMF)  
string - программа, с помощью который создавался файл (3ds Max R3 Exporter)  


MATERIALLIST  

unsigned int - количество материалов  
массив материалов  
...
string - путь до материала  

unsigned short - при изменении значения всё жёстко идёт по пизде  
...
флоты похожи на какие-то координаты или типа того  

8 байт неизвестности  

unsigned int  количество объектов на трэке  
...
unsigned int - хз  
какие-то 10 float. Похоже на что-то типа координат  
массив объектов (батчи)  


BATCH

unsigned int - индекс материала

unsigned int - количество вершин
unsigned int - количество полигонов

массив вершин:  
каждая вершина:  
3 float - координаты вершины  
3 float - нормаль вершины  
unsigned int - хз  
2 float - текстурные координаты  
2 unsigined int - хз

массив полигонов:  
каждый полигон:  
3 unsigned short - индексы вершин 
1 unsigned short - хз, как правило это ноль
...
unsigned int - количетсво объектов  
массив объектов  
...
string - имя  
float - ?  
16 float - Матрица трансформации 4 на 4  
3 float - координаты объекта  
3 float - оффсет визуальной модели объекта  
uint - ?  
10 float - ? (опять какие-то координаты как будто)  
uint - id мэша  
